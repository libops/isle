...
function create_database {
    ...
    mysql)
        ...
        ;;
    *)
        echo "Only MariaDB/MySQL databases are supported." >&2
        exit 1
        ;;
    esac
}
...
function configure_jwt_module {
    ...
    drush -l "${site_url}" -y config:import --partial --source=/etc/libops/configs/jwt
}
...
function create_blazegraph_namespace {
    ...
    curl -X POST -H "Content-type: text/plain" --data-binary @/etc/libops/configs/inference.nt "${triplestore_url}/namespace/${namespace}/sparql"
}
