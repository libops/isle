...
if is_dev_mode && has_no_docker_override; then
    print_warning_header
    echo_e "\t${YELLOW}Mount Drupal extension paths into the Drupal container for local development${RESET}"
    echo -e "\tFor easier local development, you can copy the sample docker-compose override file."
    echo -e "\tThis allows you to mount local Drupal modules and themes into the container."
    echo -e "\tTo do so you can run the following command:"
    echo_e "\t${BLUE}cp docker-compose.override-example.yaml docker-compose.override.yaml${RESET}"
