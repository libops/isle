...
if [ ! -f "$OVERRIDE_FILE" ] && [ -f "$PROJECT_ROOT/docker-compose.override.yml" ]; then
    OVERRIDE_FILE="$PROJECT_ROOT/docker-compose.override.yml"
fi

if [ ! -f "$OVERRIDE_FILE" ]; then
    echo "${RED}Error: docker-compose.override.yaml does not exist.${RESET}" >&2
    echo "" >&2
    echo "To enable mariadb port mapping, run:" >&2
    echo -e "\t${BLUE}cp docker-compose.override-example.yaml docker-compose.override.yaml" >&2
...
if ! grep -A5 'mariadb:' "$OVERRIDE_FILE" | grep -q '3306:3306'; then
    echo "${RED}Error: mariadb port 3306 is not mapped to host in docker-compose.override.yaml${RESET}" >&2
    echo "" >&2
    echo "To enable mariadb port mapping, run:" >&2
    echo -e "\t${BLUE}cp docker-compose.override-example.yaml docker-compose.override.yaml" >&2
    echo -e "\tmake up${RESET}" >&2
