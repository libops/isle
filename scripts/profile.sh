...
if [ -f .env ]; then
    ...
    export DEVELOPMENT_ENVIRONMENT ENABLE_HTTPS URI_SCHEME ENABLE_ACME ACME_EMAIL DOMAIN COMPOSE_PROJECT_NAME
else
...
has_no_docker_override() {
    status_dev || { [ ! -f docker-compose.override.yaml ] && [ ! -L docker-compose.override.yaml ] && [ ! -f docker-compose.override.yml ] && [ ! -L docker-compose.override.yml ]; }
}
